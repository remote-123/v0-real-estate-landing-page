/**
 * Ingest DLD Buildings Registry CSV into `dld_buildings_registry` table.
 * Run: npx tsx --env-file=.env.local scripts/ingest/dld_buildings_registry.ts [path/to/csv]
 */
...
const CSV_PATH =
  process.argv[2] ??
  path.join(process.cwd(), "dld_data", "dld_buildings.csv")
...
function transformRow(row: Record<string, string>) {
  return {
    ...
    area_name_ar:            nullify(row["area_name_ar"]),
    ...
    rooms:                   nullify(row["rooms"]),
    rooms_ar:                nullify(row["rooms_ar"]),
    rooms_en:                nullify(row["rooms_en"]),
    car_parks:               toInt(row["car_parks"]),
    built_up_area:           toFloat(row["built_up_area"]),
    bld_levels:              toInt(row["bld_levels"]),
    shops:                   toInt(row["shops"]),
    flats:                   toInt(row["flats"]),
    offices:                 toInt(row["offices"]),
    swimming_pools:          toInt(row["swimming_pools"]),
    elevators:               toInt(row["elevators"]),
    actual_area:             toFloat(row["actual_area"]),
    property_type_id:        toInt(row["property_type_id"]),
    property_type_ar:        nullify(row["property_type_ar"]),
    property_type_en:        nullify(row["property_type_en"]),
    property_sub_type_id:    toInt(row["property_sub_type_id"]),
    property_sub_type_ar:    nullify(row["property_sub_type_ar"]),
    ...
    master_project_ar:       nullify(row["master_project_ar"]),
    project_id:              toInt(row["project_id"]),
    project_name_ar:         nullify(row["project_name_ar"]),
    project_name_en:         nullify(row["project_name_en"]),
    land_type_id:            toInt(row["land_type_id"]),
    land_type_ar:            nullify(row["land_type_ar"]),
...
async function ingest() {
  ...
  const flush = async () => {
    ...
    try {
      await sql`INSERT INTO dld_buildings_registry ${sql(batch)} ON CONFLICT (property_id) DO NOTHING`
