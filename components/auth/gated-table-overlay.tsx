...
export function GatedTableOverlay({ freeRows, totalRows }: Props) {
  return (
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 flex flex-col items-center gap-3 pb-4 pt-8">
        <div className="flex items-center gap-2 text-muted-foreground text-sm">
          ...
          <span>
            ...
            <span className="font-semibold text-foreground">{totalRows}</span> communities
          </span>
        </div>
        <Button asChild size="sm" className="bg-accent text-accent-foreground hover:bg-accent/90 gap-2">
          <Link href="/sign-in?callbackUrl=/terminal/communities">
