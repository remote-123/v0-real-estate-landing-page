...
const AUTOMATIONS: Automation[] = [
  ...
  {
    name: 'Prop Areas',
    route: '/api/cron/prop-areas',
    trigger: 'cron-job.org',
    schedule: 'Weekly',
    what: 'Scrapes propsearch.ae area list (pages 1–2), upserts area slugs into prop_areas table',
    outputs: 'prop_areas',
    auth: 'Bearer CRON_SECRET',
    external: ['propsearch.ae'],
    status: 'active',
  },
  {
    name: 'Prop Buildings',
    route: '/api/cron/prop-buildings',
    trigger: 'cron-job.org',
    schedule: 'Daily',
    what: 'For each area in prop_areas, fetches building list from propsearch.ae and upserts into prop_buildings',
    outputs: 'prop_buildings, prop_areas.total_buildings',
    auth: 'Bearer CRON_SECRET',
    external: ['propsearch.ae'],
    status: 'active',
  },
  {
    name: 'Prop Building Details',
    route: '/api/cron/prop-building-details',
    trigger: 'cron-job.org',
    schedule: 'Daily',
    what: 'Fetches detail pages for buildings in prop_buildings that need enrichment (floor count, developer, year, amenities)',
    outputs: 'prop_buildings (enriched)',
    auth: 'Bearer CRON_SECRET',
    external: ['propsearch.ae'],
    status: 'active',
  },
