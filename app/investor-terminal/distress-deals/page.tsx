...
import { DistressFeedCard } from "@/components/investor-terminal/distress-feed-card"
import { DistressFilters } from "@/components/investor-terminal/distress-filters"
...
async function fetchBayutDeals() {
    ...
    const options = {
        ...
        next: { revalidate: 43200 } // Cache Bayut API response for 12 hours
    }
    try {
...
async function fetchPropertyFinderDeals() {
    ...
    const options = {
        ...
        next: { revalidate: 43200 } // Cache Property Finder API response for 12 hours
    }
    try {
        const res = await fetch(url, options)
    ...
    return (
        <div className="flex w-full flex-col p-6 sm:p-8 xl:p-12 space-y-10 max-w-7xl mx-auto pb-24 lg:pb-12">
            ...
            <section className="space-y-4 pb-20">
                <h3 className="font-mono text-sm tracking-widest text-muted-foreground uppercase pb-2 border-b border-border/50 flex items-center justify-between">
